...
@Unique(
        name = "Producto__name__UNQ", members = { "nombre" }
)
@Queries({
        @Query(
                name = Producto.NAMED_QUERY__FIND_BY_NAME_LIKE,
                value = "SELECT " +
                        "FROM domainapp.modules.simple.dom.so.Producto " +
                        "WHERE name.indexOf(:name) >= 0"
        ),
        @Query(
                name = Producto.NAMED_QUERY__FIND_BY_NAME_EXACT,
                value = "SELECT " +
                        "FROM domainapp.modules.simple.dom.so.Producto " +
                        "WHERE name == :name"
        )
...
@Named(SimpleModule.NAMESPACE + ".Producto")
...
public class Producto implements Comparable<Producto> {
    ...
    static final String NAMED_QUERY__FIND_BY_NAME_LIKE = "Producto.findByNameLike";
    static final String NAMED_QUERY__FIND_BY_NAME_EXACT = "Producto.findByNameExact";

    public static Producto withName(final String nombre, final int precio, final int stock, final int cantidad) {
...
//    }
    ...
    private final static Comparator<Producto> comparator =
